...
def nav_nonDP(split, env, episode_id, scene_name, scene_height, start_pose, saved_folder, device):
	...
	while step < cfg.NAVI.NUM_STEPS:
		...
		#=============================== check if we need to save the middle results =================#
		if step == cfg.EVAL.MIDDEL_STEP_SAVE:
			_, middle_observed_area_flag, _ = semMap_module.get_semantic_map()
	...
	#==================================== concatenate with the middle results =================
	if middle_observed_area_flag is not None:
		observed_area_flag = np.stack((observed_area_flag, middle_observed_area_flag))
