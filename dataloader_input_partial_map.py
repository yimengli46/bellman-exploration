...
class MP3DSceneDataset(data.Dataset):
    ...
    def __init__(self, split, scene_name, data_folder=''):
        ...
        if self.split == 'val':
            self.sample_name_list = [os.path.splitext(os.path.basename(
                x))[0] for x in sorted(glob.glob(f'{self.saved_folder}/*.pbz2'))]
    ...
    def __getitem__(self, i):
        ...
        if cfg.PRED.PARTIAL_MAP.FLAG_VISUALIZE_PRED_LABELS:
            ...
            for f in frontiers:
                ax[1][0].scatter(f.points[1], f.points[0],
                                 c='yellow', zorder=2)
                ax[1][0].scatter(f.centroid[1], f.centroid[0],
                                 c='red', zorder=2)
        ...
        resized_U = np.zeros(
            (4, cfg.PRED.PARTIAL_MAP.INPUT_WH[1], cfg.PRED.PARTIAL_MAP.INPUT_WH[0]), dtype=np.float32)
        resized_U[0] = U_a
        resized_U[1] = U_d[:, :, 0]
        resized_U[2] = U_d[:, :, 1]
        resized_U[3] = U_d[:, :, 2]
...
if __name__ == "__main__":
    ...
    data_folder = 'output/training_data_input_partial_map_5000samples'
