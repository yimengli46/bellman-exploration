...
def get_frontiers(occupancy_grid, gt_occupancy_grid, observed_area_flag):
    ...
    frontier_point_mask = np.logical_and(
        filtered_grid, occupancy_grid == cfg.FE.FREE_VAL)
    ...
    frontiers = set()
    for ii in range(nb):
        raw_frontier_indices = np.where(np.logical_and(
            labels == (ii + 1), frontier_point_mask))
        frontiers.add(
            fr_utils.Frontier(
...
occ_map_npy = np.load(
    f'{cfg.SAVE.OCCUPANCY_MAP_PATH}/test/{scene_name}/BEV_occupancy_map.npy', allow_pickle=True).item()
...
gt_occ_map = np.where(gt_occ_map == 1, cfg.FE.FREE_VAL,
                      gt_occ_map)  # free cell
gt_occ_map = np.where(gt_occ_map == 0, cfg.FE.COLLISION_VAL,
                      gt_occ_map)  # occupied cell
...
ax.scatter(agent_map_pose[0], agent_map_pose[1],
           marker='s', s=50, c='red', zorder=5)
...
labels, nb = scipy.ndimage.label(
    local_occupancy_map, structure=np.ones((3, 3)))
